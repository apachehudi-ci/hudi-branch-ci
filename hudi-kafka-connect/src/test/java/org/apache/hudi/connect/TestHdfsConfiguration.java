...
public class TestHdfsConfiguration {
  ...
  public void testHadoopConfigEnvs() throws Exception {
    List<Path> paths = withEnvironmentVariable(
        "HADOOP_CONF_DIR", "src/test/resources/hadoop_conf")
        .and("HADOOP_HOME", "")
        .execute(() -> KafkaConnectUtils.getHadoopConfigFiles(
  ...
  public void testHadoopHomeEnvs() throws Exception {
    List<Path> paths = withEnvironmentVariable(
        "HADOOP_CONF_DIR", "")
        .and("HADOOP_HOME", "src/test/resources/hadoop_home")
        .execute(() -> KafkaConnectUtils.getHadoopConfigFiles(
                System.getenv("HADOOP_CONF_DIR"),
                System.getenv("HADOOP_HOME")
        ));
