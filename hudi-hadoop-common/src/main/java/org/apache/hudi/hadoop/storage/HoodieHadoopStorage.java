...
public class HoodieHadoopStorage implements HoodieStorage {
  ...
  @Override
  public void createImmutableFileInPath(HoodieLocation location,
                                        Option<byte[]> content) throws IOException {
    // TODO
    try (OutputStream stream = fs.create(convertHoodieLocationToPath(location))) {
      if (content.isPresent()) {
        stream.write(content.get());
      }
    }
  }
  ...
  @Override
  public HoodieFileStatus getFileInfo(HoodieLocation location) throws IOException {
  ...
  @Override
  public List<HoodieFileStatus> listDirectEntries(HoodieLocation location, HoodieLocationFilter filter)
      throws IOException {
