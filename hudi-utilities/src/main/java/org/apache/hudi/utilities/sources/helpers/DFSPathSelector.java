...
public class DFSPathSelector implements Serializable {
  ...
  public DFSPathSelector(TypedProperties props, Configuration hadoopConf) {
    ...
    this.props = props;
    this.storage = FSUtils.getHoodieStorage(
  ...
  public Pair<Option<String>, String> getNextFilePathsAndMaxModificationTime(Option<String> lastCheckpointStr,
    ...
    try {
      ...
      List<HoodieFileStatus> eligibleFiles = listEligibleFiles(
          storage, new HoodieLocation(getStringWithAltKeys(props,
              DFSPathSelectorConfig.ROOT_INPUT_PATH)),
          lastCheckpointTime);
      // sort them by modification time.
      eligibleFiles.sort(Comparator.comparingLong(HoodieFileStatus::getModificationTime));
      // Filter based on checkpoint & input size, if needed
      long currentBytes = 0;
      long newCheckpointTime = lastCheckpointTime;
      List<HoodieFileStatus> filteredFiles = new ArrayList<>();
      for (HoodieFileStatus f : eligibleFiles) {
   ...
   */
  protected List<HoodieFileStatus> listEligibleFiles(HoodieStorage storage, HoodieLocation path,
                                                     long lastCheckpointTime) throws IOException {
    // skip files/dirs whose names start with (_, ., etc)
    List<HoodieFileStatus> statuses = storage.listDirectEntries(path, file ->
        IGNORE_FILEPREFIX_LIST.stream().noneMatch(pfx -> file.getName().startsWith(pfx)));
    List<HoodieFileStatus> res = new ArrayList<>();
    for (HoodieFileStatus status : statuses) {
      if (status.isDirectory()) {
        // avoid infinite loop
        if (!status.isSymlink()) {
          res.addAll(listEligibleFiles(storage, status.getLocation(), lastCheckpointTime));
        }
      } else if (status.getModificationTime() > lastCheckpointTime && status.getLength() > 0) {
        res.add(status);
      }
    }
    return res;
