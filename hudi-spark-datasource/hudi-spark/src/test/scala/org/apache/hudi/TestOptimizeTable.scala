...
class TestOptimizeTable extends HoodieClientTestBase {
  ...
  def testOptimizewithwriter(tableType: String): Unit = {
    ...
    // do optimize
    import org.apache.hudi.toDataFrameFunctions
    val originDF = spark.read.format("hudi").load(basePath)

    val options = commonOpts ++ Map(DataSourceWriteOptions.OPERATION.key() -> "optimize",
      HoodieOptimizeConfig.DATA_LAYOUT_DATA_SKIPPING_ENABLE.key -> "true")

    originDF.optimizeByZOrder(Seq("begin_lat", "begin_lon"), options, outputFileNum = 4)
  ...
  def testOptimizewithClustering(tableType: String): Unit = {
    ...
    inputDF1.write.format("org.apache.hudi")
      ...
      .option(HoodieOptimizeConfig.DATA_LAYOUT_CURVE_OPTIMIZE_SORT_COLUMNS.key, "begin_lat, begin_lon")
      .mode(SaveMode.Overwrite)
      .save(basePath)
