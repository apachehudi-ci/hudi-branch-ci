...
class TestHdfsParquetImportProcedure extends HoodieSparkProcedureTestBase {
  ...
  test("Test Call hdfs_parquet_import Procedure with insert operation") {
    withTempDir { tmp =>
      val storage: HoodieStorage = FSUtils.getHoodieStorage(tmp.getCanonicalPath, spark.sparkContext.hadoopConfiguration)
      val tableName = generateTableName
      val tablePath = tmp.getCanonicalPath + PATH_SEPARATOR + tableName
      ...
      // create schema file
      val schemaFileOS = storage.create(new HoodieLocation(schemaFile))
  ...
  test("Test Call hdfs_parquet_import Procedure with upsert operation") {
    withTempDir { tmp =>
      val storage: HoodieStorage = FSUtils.getHoodieStorage(
