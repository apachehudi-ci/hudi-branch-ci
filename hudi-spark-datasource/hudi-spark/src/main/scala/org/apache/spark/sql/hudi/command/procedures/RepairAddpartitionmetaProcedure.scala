...
class RepairAddpartitionmetaProcedure extends BaseProcedure with ProcedureBuilder with Logging {
  ...
  override def call(args: ProcedureArgs): Seq[Row] = {
    ...
    val partitionPaths: util.List[String] = FSUtils.getAllPartitionFoldersThreeLevelsDown(metaClient.getHoodieStorage, tablePath);
    ...
    for (partition <- partitionPaths) {
      val partitionPath: HoodieLocation = FSUtils.getPartitionPath(basePath, partition)
      var isPresent = "Yes"
      var action = "None"
      if (!HoodiePartitionMetadata.hasPartitionMetadata(metaClient.getHoodieStorage, partitionPath)) {
        isPresent = "No"
        if (!dryRun) {
          val partitionMetadata: HoodiePartitionMetadata = new HoodiePartitionMetadata(
            metaClient.getHoodieStorage, latestCommit, basePath, partitionPath, metaClient.getTableConfig.getPartitionMetafileFormat)
