...
object HoodieAnalysis {
  ...
  private def instantiateKlass(nestedSchemaPruningClass: String): Rule[LogicalPlan] = {
    loadClass(nestedSchemaPruningClass).asInstanceOf[Rule[LogicalPlan]]
  }

  private def instantiateKlass(dataSourceV2ToV1FallbackClass: String, session: SparkSession): Rule[LogicalPlan] = {
    ...
    loadClass(dataSourceV2ToV1FallbackClass, Array(classOf[SparkSession]).asInstanceOf[Array[Class[_]]], session)
