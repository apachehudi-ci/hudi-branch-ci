...
class ExportInstantsProcedure extends BaseProcedure with ProcedureBuilder with Logging {
  ...
  private def copyArchivedInstants(basePath: String, statuses: util.List[FileStatus], actionSet: util.Set[String], limit: Int, localFolder: String) = {
    ...
    var copyCount = 0
    val storage = FSUtils.getHoodieStorage(basePath, jsc.hadoopConfiguration())
    for (fs <- statuses) {
      // read the archived file
      val reader = HoodieLogFormat.newReader(
        storage, new HoodieLogFile(new HoodieLocation(fs.getPath.toUri)), HoodieArchivedMetaEntry.getClassSchema)
      ...
      while ( {
        ...
        try {
          ...
          try while ( {
            ...
            val outPath = localFolder + PATH_SEPARATOR + instantTime + "." + action
  ...
  private def copyNonArchivedInstants(metaClient: HoodieTableMetaClient, instants: util.List[HoodieInstant], limit: Int, localFolder: String): Int = {
    ...
    if (instants.nonEmpty) {
      ...
      val storage = FSUtils.getHoodieStorage(metaClient.getBasePath, jsc.hadoopConfiguration())
      for (instant <- instants) {
        val localPath = localFolder + PATH_SEPARATOR + instant.getFileName
  ...
  private def writeToFile(storage: HoodieStorage, path: String, data: Array[Byte]): Unit = {
    val out = storage.create(new HoodieLocation(path))
    out.write(data)
    out.flush()
    out.close()
  }
