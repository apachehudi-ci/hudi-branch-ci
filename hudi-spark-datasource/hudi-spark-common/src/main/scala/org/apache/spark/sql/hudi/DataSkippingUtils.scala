...
object DataSkippingUtils extends Logging {
  ...
  private def tryComposeIndexFilterExpr(sourceFilterExpr: Expression, indexSchema: StructType): Option[Expression] = {
    ...
    sourceFilterExpr match {
      ...
      // Translates to "colA_nullCount < colA_valueCount or colA_nullCount = null" for index lookup
      // "colA_nullCount = null" means we are not certain whether the column is null or not,
      ...
      case IsNotNull(attribute: AttributeReference) =>
        getTargetIndexedColumnName(attribute, indexSchema)
          .map(colName => Or(
            ...
            EqualTo(genColNumNullsExpr(colName), Literal(null, attribute.dataType))
          ))
