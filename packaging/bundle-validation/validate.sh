...
UTILITIES_DIR=${WORKDIR}/utilities
...
test_spark_bundle () {
    ...
    $SPARK_HOME/bin/spark-shell --jars $JARS_DIR/spark.jar < validate.scala
    if [ "$?" -ne 0 ]; then
...
# Inputs:
#   SPARK_HOME: path to the spark directory
#   UTILITIES_DIR: path to the directory where the files used in testing the
...
test_utilities_bundle () {
    ...
    $OPT_JARS $MAIN_JAR \
    --props $UTILITIES_DIR/hoodieapp.properties \
...
then
  ...
  rm -rf $OUTPUT_DIR
  COMMANDS_FILE=$UTILITIES_DIR/commands.scala
  test_utilities_bundle
  if [ "$?" -ne 0 ]; then
      exit 1
  fi
...
rm -rf $OUTPUT_DIR
COMMANDS_FILE=$UTILITIES_DIR/slimcommands.scala
test_utilities_bundle
if [ "$?" -ne 0 ]; then
    exit 1
fi
