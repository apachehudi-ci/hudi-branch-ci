...
public class HoodieIndexConfig extends HoodieConfig {
  ...
  public static final ConfigProperty<Integer> BUCKET_INDEX_BUCKET_NUM = ConfigProperty
  ...
  public static class Builder {
    ...
    private void validateBucketIndexConfig() {
      if (hoodieIndexConfig.getString(INDEX_TYPE)
        ...
        if (StringUtils.isNullOrEmpty(hoodieIndexConfig.getString(INDEX_KEY_FILED_NAME))) {
          throw new HoodieIndexException("When using bucket index, hoodie.index.key.field cannot be null/empty.");
        } else if (hoodieIndexConfig.getIntOrDefault(BUCKET_INDEX_BUCKET_NUM) <= 0) {
          throw new HoodieIndexException("When using bucket index, hoodie.index.bucket.num cannot be negative.");
        }
      }
    }
  }
