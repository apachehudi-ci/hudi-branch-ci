...
public abstract class HoodieBackedTableMetadataWriter<I> implements HoodieTableMetadataWriter {
  ...
  private List<DirectoryInfo> listAllPartitionsFromMDT(String initializationTime) throws IOException {
    ...
    Map<String, FileStatus[]> partitionFileMap = metadata.getAllFilesInPartitions(allPartitionPaths)
        ...
        .collect(Collectors.toMap(e -> e.getKey(),
            e -> e.getValue().stream().map(info -> FSUtils.convertFileInfoToFileStatus(info)).toArray(FileStatus[]::new)));
  ...
  private void initializeFileGroups(HoodieTableMetaClient dataMetaClient, MetadataPartitionType metadataPartition, String instantTime,
    ...
    final HoodieLocation partitionPath = new HoodieLocation(metadataWriteConfig.getBasePath(), partitionName);
    HoodieStorage storage = metadataMetaClient.getHoodieStorage();
    try {
      final List<HoodieFileStatus> existingFiles = storage.listDirectEntries(partitionPath);
    ...
    engineContext.foreach(fileGroupFileIds, fileGroupFileId -> {
      try {
        ...
        HoodieLogFormat.Writer writer = HoodieLogFormat.newWriterBuilder()
            .onParentPath(FSUtils.getPartitionPath(
                new HoodieLocation(metadataWriteConfig.getBasePath()), partitionName))
            .withFileId(fileGroupFileId)
            .withDeltaCommit(instantTime)
            .withLogVersion(HoodieLogFile.LOGFILE_BASE_VERSION)
            .withFileSize(0L)
            .withSizeThreshold(metadataWriteConfig.getLogFileMaxSize())
            .withHoodieStorage(dataMetaClient.getHoodieStorage())
  ...
  public void dropMetadataPartitions(List<MetadataPartitionType> metadataPartitions) throws IOException {
    for (MetadataPartitionType partitionType : metadataPartitions) {
      ...
      dataMetaClient.getHoodieStorage()
          .deleteDirectory(new HoodieLocation(metadataWriteConfig.getBasePath(), partitionPath));
  ...
  private void fetchOutofSyncFilesRecordsFromMetadataTable(Map<String, DirectoryInfo> dirInfoMap, Map<String, Map<String, Long>> partitionFilesToAdd,
    ...
    for (String partition : metadata.fetchAllPartitionPaths()) {
      HoodieLocation partitionPath = null;
      if (StringUtils.isNullOrEmpty(partition) && !dataMetaClient.getTableConfig().isTablePartitioned()) {
        partitionPath = new HoodieLocation(dataWriteConfig.getBasePath());
      } else {
        partitionPath = new HoodieLocation(dataWriteConfig.getBasePath(), partition);
      }
      final String partitionId = HoodieTableMetadataUtil.getPartitionIdentifierForFilesPartition(partition);
      List<HoodieFileStatus> metadataFiles = metadata.getAllFilesInPartition(partitionPath);
      if (!dirInfoMap.containsKey(partition)) {
        ...
        if (metadataFiles != null && metadataFiles.size() > 0) {
          partitionFilesToDelete.put(partitionId, metadataFiles.stream().map(f -> f.getLocation().getName()).collect(Collectors.toList()));
        }
      } else {
        ...
        List<String> mdtFiles = metadataFiles.stream().map(mdtFile -> mdtFile.getLocation().getName()).collect(Collectors.toList());
        List<String> filesDeleted = metadataFiles.stream().map(f -> f.getLocation().getName())
