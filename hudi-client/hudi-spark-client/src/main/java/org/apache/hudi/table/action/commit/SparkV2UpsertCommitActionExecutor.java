...
import static org.apache.hudi.client.utils.SparkDataFrameWriterUtils.toMap;
...
public class SparkV2UpsertCommitActionExecutor implements Serializable {
  ...
  protected final HoodieBaseTable<Dataset<Row>, Dataset<HoodieKey>, Dataset<HoodieInternalWriteStatus>> table;
  ...
  public SparkV2UpsertCommitActionExecutor(HoodieSparkEngineContext context,
      ...
      HoodieBaseTable<Dataset<Row>, Dataset<HoodieKey>, Dataset<HoodieInternalWriteStatus>> table,
  ...
  public HoodieWriteMetadata<Dataset<HoodieInternalWriteStatus>> execute() {
  ...
  public HoodieWriteMetadata<Dataset<HoodieInternalWriteStatus>> execute(Dataset<Row> inputDf) {
    inputDf.cache();
    ...
    Dataset<HoodieInternalWriteStatus> writeStatuses = clusteringHandleUpdate(inputDf)
        .mapPartitions((MapPartitionsFunction<Row, HoodieInternalWriteStatus>) rows -> {
          try (HoodieRowMergeHandle mergeHandle = new HoodieRowMergeHandle(schema,
        ...
        }, Encoders.bean(HoodieInternalWriteStatus.class));

    HoodieWriteMetadata<Dataset<HoodieInternalWriteStatus>> result = new HoodieWriteMetadata<>();
