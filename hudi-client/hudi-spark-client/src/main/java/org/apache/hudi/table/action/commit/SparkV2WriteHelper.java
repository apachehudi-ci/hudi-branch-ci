...
public class SparkV2WriteHelper {
  ...
  public HoodieWriteMetadata<Dataset<HoodieInternalWriteStatus>> write(
      String instantTime,
      Dataset<Row> inputDf,
    ...
    HoodieWriteMetadata<Dataset<HoodieInternalWriteStatus>> result = executor.execute(taggedDf);
    result.setIndexLookupDuration(indexLookupDuration);
    return result;
  }

  private Dataset<Row> combineOnCondition(
      boolean condition, Dataset<Row> df, int parallelism, HoodieBaseTable<Dataset<Row>, Dataset<HoodieKey>, Dataset<HoodieInternalWriteStatus>> table) {
