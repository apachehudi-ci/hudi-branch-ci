...
public class RDDSpatialCurveOptimizationSortPartitioner<T extends HoodieRecordPayload>
  ...
  private JavaRDD<GenericRecord> prepareGenericRecord(JavaRDD<HoodieRecord<T>> inputRecords, final int numOutputGroups, final Schema schema) {
    ...
    if (config.getOptimizeBuildCurveOptimizeMethod().equals("sample")) {
      zDataFrame = Zoptimize$.MODULE$.createZIndexedDataFrameBySample(originDF, config.getOptimizeSortColumns(), numOutputGroups);
    } else {
      zDataFrame = Zoptimize$.MODULE$.createZIndexedDataFrameByMapValue(originDF, config.getOptimizeSortColumns(), numOutputGroups);
    }
