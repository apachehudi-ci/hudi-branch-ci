...
object Zoptimize {
  ...
  def createZIndexedDataFrameBySample(df: DataFrame, zCols: Seq[String], fileNum: Int): DataFrame = {
    ...
    if (zCols.isEmpty || checkCols.isEmpty) {
      ...
      val zOrderBounds = df.sparkSession.sessionState.conf.getConfString(
        HoodieOptimizeConfig.DATA_LAYOUT_CURVE_OPTIMIZE_SAMPLE_NUMBER.key,
        HoodieOptimizeConfig.DATA_LAYOUT_CURVE_OPTIMIZE_SAMPLE_NUMBER.defaultValue.toString).toInt
  ...
  def getMinMaxValue(df: DataFrame, zCols: String): DataFrame = {
    ...
    val computeParallelismThreshold = conf.getInt("spark.hudi.computeParallelismThreshold", 100)

    val startTime = System.nanoTime()

    val allMetaData: Array[ColumnFileStats] = if (inputFiles.length < computeParallelismThreshold) {
  ...
  def saveStatisticsInfo(
    ...
    if (fs.exists(new Path(indexPath))) {
      val candidateIndexTables = fs.listStatus(new Path(indexPath)).filter(_.isDirectory)
        .map(_.getPath.getName).filter(f => validateCommits.contains(f)).sortBy(x => x).toList

      val optIndexDf = if (candidateIndexTables.isEmpty) {
        None
      } else {
        try {
          Some(spark.read.load(indexPath))
        } catch {
          case _: Throwable =>
            None
        }
      ...
      if (optIndexDf.isDefined && optIndexDf.get.schema.equals(statisticsDF.schema) && saveMode.equals("append")) {
